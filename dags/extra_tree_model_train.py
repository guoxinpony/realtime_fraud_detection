...
class ExtraTreeFraudTrainer:
    """Train an Extra Trees classifier for fraud detection using Kafka sourced data."""
    ...
    def __init__(self, config_path: Optional[str] = None, sample_size: Optional[int] = None) -> None:
        ...
        self.model_config: Dict[str, object] = models_config.get("extra_tree", {})
        if not self.model_config:
            raise ValueError("Configuration is missing `models.extra_tree` settings.")
    ...
    def train_model(self) -> Tuple[Pipeline, Dict[str, float]]:
        logger.info("Starting Extra Trees fraud detection training workflow.")
        ...
        default_path = "/app/models/fraud_detection_model_extra_tree.pkl"
