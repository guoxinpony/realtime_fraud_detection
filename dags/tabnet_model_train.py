...
class TabNetFraudTrainer:
    """Train a TabNet classifier for fraud detection using Kafka sourced data."""
    ...
    def __init__(self, config_path: Optional[str] = None, sample_size: Optional[int] = None) -> None:
        ...
        self.model_config: Dict[str, object] = models_config.get("tabnet", {})
        if not self.model_config:
            raise ValueError("Configuration is missing `models.tabnet` settings.")
        ...
        if model_max_messages is not None:
            try:
                ...
                raise ValueError(f"Invalid models.tabnet.max_messages value: {model_max_messages}") from exc
    ...
    def train_model(self) -> Tuple[Pipeline, Dict[str, float]]:
        logger.info("Starting TabNet fraud detection training workflow.")
        ...
        model_path = self._resolve_path(str(self.model_config.get("path", "/app/models/fraud_detection_model_tabnet.pkl")))
