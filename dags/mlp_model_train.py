...
class MLPFraudTrainer:
    """Train an MLP classifier for fraud detection using Kafka sourced data."""
    ...
    def __init__(self, config_path: Optional[str] = None, sample_size: Optional[int] = None) -> None:
        ...
        self.model_config: Dict[str, object] = models_config.get("mlp", {})
        if not self.model_config:
            raise ValueError("Configuration is missing `models.mlp` settings.")
    ...
    def train_model(self) -> Tuple[Pipeline, Dict[str, float]]:
        logger.info("Starting MLP fraud detection training workflow.")
        df = self.read_from_kafka()
        df = self._augment_features(df)

        features = df.drop(columns=["is_fraud", "transaction_time"]).reset_index(drop=True)
        target = df["is_fraud"].reset_index(drop=True)
        ...
        default_path = "/app/models/fraud_detection_model_mlp.pkl"
