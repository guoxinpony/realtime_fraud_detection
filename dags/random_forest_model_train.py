...
class RandomForestFraudTrainer:
    """Train a RandomForest classifier for fraud detection using Kafka sourced data."""
    ...
    def __init__(self, config_path: Optional[str] = None, sample_size: Optional[int] = None) -> None:
        ...
        self.model_config: Dict[str, object] = models_config.get("random_forest", {})
        if not self.model_config:
            raise ValueError("Configuration is missing `models.random_forest` settings.")
        ...
        self.noise_level = float(self.model_config.get("noise_level", 0.05))
    ...
    def _add_gaussian_noise(self, df: pd.DataFrame, seed: int) -> pd.DataFrame:
        """Inject Gaussian noise into numeric columns to encourage robustness."""
        if df.empty:
            return df
        ...
        std = df_noisy[numeric_cols].std(ddof=0)
        std.replace(0, 1e-8, inplace=True)
        noise_scale = std.to_numpy() * self.noise_level
        noise = rng.normal(loc=0.0, scale=noise_scale, size=df_noisy[numeric_cols].shape)
        df_noisy.loc[:, numeric_cols] = df_noisy[numeric_cols].to_numpy() + noise
        return df_noisy

    def train_model(self) -> Tuple[Pipeline, Dict[str, float]]:
        logger.info("Starting RandomForest fraud detection training workflow.")
        ...
        with mlflow.start_run():
            ...
            cv_auc_scores = []
            for fold_index, (train_idx, val_idx) in enumerate(cv.split(features, target), start=1):
                X_fold_train = features.iloc[train_idx]
                y_fold_train = target.iloc[train_idx]
                X_fold_val = features.iloc[val_idx]
                y_fold_val = target.iloc[val_idx]

                noise_seed = seed + fold_index
                X_fold_train_noisy = self._add_gaussian_noise(X_fold_train, noise_seed)

                fold_pipeline = clone(pipeline)
                fold_pipeline.fit(X_fold_train_noisy, y_fold_train)

                y_val_proba = fold_pipeline.predict_proba(X_fold_val)[:, 1]
                cv_auc_scores.append(roc_auc_score(y_fold_val, y_val_proba))

            cv_auc_scores = np.array(cv_auc_scores, dtype=float)
            cv_auc_mean = float(cv_auc_scores.mean())
            cv_auc_std = float(cv_auc_scores.std(ddof=0))
            mlflow.log_metric("cv_roc_auc_mean", cv_auc_mean)
            mlflow.log_metric("cv_roc_auc_std", cv_auc_std)

            X_train_noisy = self._add_gaussian_noise(X_train, seed)
        ...
        model_path = self._resolve_path(str(self.model_config.get("path", "/app/models/fraud_detection_model_rf.pkl")))
